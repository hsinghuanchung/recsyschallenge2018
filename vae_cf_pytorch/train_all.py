...
def main(args):
    ...
    if(args.loadModel != ""):
        ...
        vae.load_state_dict(torch.load(args.loadModel+'vae_params_all.pkl'))
            ...
            if(index+1) % 2000 == 0:   #check the performance every 3000*args.batchSize
                ...
                with torch.no_grad():
                    for _,sample_val in enumerate(dataloader):
                        ...
                        ndcg.append(eval.NDCG_binary_at_k_batch(output_val,ans_val))
                        rpre.extend(eval.Rpre_at_k_batch(output_val,ans_val,length_val,size))
                        qqq += 1
                        if(qqq==80):
                            break
                ...
                if(best_ndcg < np.mean(ndcg) or best_rpre < np.mean(rpre)):
                    torch.save(vae.state_dict(), args.saveModel+'vae_params_all.pkl') # save only the parameters
