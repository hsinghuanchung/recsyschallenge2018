...
def load_data(data_file):
    ...
    for line in tp:
        ...
        line = line[1:]
        line = [ int(_) for _ in line if( int(_) ) in unique_sid]

        indptr_list.append(indptr_list[-1]+len(line))
...
test_data = load_data(os.path.join(pro_dir, 'input_num6.in'))

N = test_data.shape[0]
idxlist_test = range(N)
batch_size_test = 500
...
with tf.Session() as sess:
    ...
    for bnum, st_idx in enumerate(range(0, N, batch_size_test)):
        print(bnum)
        end_idx = min(st_idx + batch_size_test, N)
        ...
        ans_output(pred_val, k=500,file_ans=f_ans)
